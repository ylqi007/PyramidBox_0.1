...
def configure_learning_rate(flags, num_samples_per_epoch, global_step):
    ...
    elif flags.learning_rate_decay_type == 'triangular':
        base_lr = 0.0001
        max_lr = 0.001
        step_size = np.floor(5011 / (2 * 32))
        cycle = tf.floor(1 + tf.cast(tf.divide(global_step, 2 * step_size), tf.float32))
        x = tf.abs(tf.cast(tf.divide(global_step, step_size), dtype=tf.float32) - 2 * cycle + 1)
...
# =========================================================================== #
def triangular_learning_rate(global_steps, base_lr, max_lr, flags):
    step_size = np.floor(5011 / (2 * 32))
    cycle = tf.floor(1 + tf.cast(tf.divide(global_steps, 2*step_size), tf.float32))
    x = tf.abs(tf.cast(tf.divide(global_steps, step_size), dtype=tf.float32) - 2 * cycle + 1)
    learning_rate = base_lr + (max_lr - base_lr) * tf.maximum(0., (1 - x))
    return learning_rate
