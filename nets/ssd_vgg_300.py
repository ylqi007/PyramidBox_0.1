...
def ssd_anchor_one_layer(img_shape,
    ...
    Arguments:
        ...
        anchor_sizes: Absolute reference sizes;
        anchor_ratios: Ratios to use on these features;
        anchor_step:
        offset: Grid offset.
    Return:
...
def ssd_anchors_all_layers(img_shape,
    ...
    for i, layer_shape in enumerate(layers_shape):
        anchor_bboxes = ssd_anchor_one_layer(img_shape,
                                             layer_shape,       # shape of feature layer, e.g. (38, 38)
                                             anchor_sizes[i],   # anchor size, e.g. (21., 45.)
                                             anchor_ratios[i],  # [2, .5, 3., 1./3]
                                             anchor_steps[i],   # e.g. 8
                                             offset=offset,     # 0.5
                                             dtype=dtype)
...
def ssd_net(inputs,
    ...
    with tf.compat.v1.variable_scope(scope, 'ssd_300_vgg', [inputs], reuse=reuse):
        ...
        print('#### inputs: ', inputs, type(inputs), inputs.shape)
        net = slim.repeat(inputs, 2, slim.conv2d, 8, [3, 3], scope='conv1')
